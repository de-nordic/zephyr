...
static ALWAYS_INLINE int flash_img_init_internal(struct flash_img_context *ctx,
						 const struct flash_area *area)
{
	...
	return stream_flash_init(&ctx->stream, flash_area_get_device(ctx->flash_area),ctx->buf,
...
{
	return flash_img_init_internal(ctx, area);
}

int flash_img_init_default(struct flash_img_context *ctx)
{

	return flash_img_init_internal(ctx, FIXED_PARTITION(UPLOAD_FLASH_AREA_LABEL));
}
		    ...
		    const struct flash_area *area)
{
	...
	return flash_area_check_int_sha256(area, &fac);
}
#endif
