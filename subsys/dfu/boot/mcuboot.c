...
#define RUNNING_FLASH_IMG_ID DT_FIXED_PARTITION_ID(DT_CHOSEN(zephyr_code_partition))
#if DT_HAS_CHOSEN(zephyr_dfu_code_partition)
#define UPDATEABLE_FLASH_IMG_ID DT_FIXED_PARTITION_ID(DT_CHOSEN(zephyr_dfu_code_partition))
#endif
...
#endif
	case RUNNING_FLASH_IMG_ID:
#ifdef UPDATEABLE_FLASH_IMG_ID
	case UPDATEABLE_FLASH_IMG_ID:
#endif
...
int mcuboot_swap_type(void)
{
	...
	rc = boot_read_swap_state_by_id(RUNNING_FLASH_IMG_ID,
					&primary_slot);
	if (rc) {
		return rc;
	}

	rc = boot_read_swap_state_by_id(UPDATEABLE_FLASH_IMG_ID,
					&secondary_slot);
	if (rc) {
		return rc;
	}
...
int boot_request_upgrade(int permanent)
{
	...
	rc = boot_magic_write(UPDATEABLE_FLASH_IMG_ID);
	if (rc) {
		goto op_end;
	}

	if (permanent) {
		rc = boot_image_ok_write(UPDATEABLE_FLASH_IMG_ID);
	...
	rc = boot_swap_type_write(UPDATEABLE_FLASH_IMG_ID, swap_type);
#else
	}
#endif
op_end:
...
{
	return boot_image_ok_read(RUNNING_FLASH_IMG_ID) == BOOT_FLAG_SET;
}
...
int boot_write_img_confirmed(void)
{
	...
	if (boot_image_ok_read(RUNNING_FLASH_IMG_ID) !=
	    erased_flag_val(RUNNING_FLASH_IMG_ID)) {
		/* Already confirmed. */
		return 0;
	}

	rc = boot_image_ok_write(RUNNING_FLASH_IMG_ID);
