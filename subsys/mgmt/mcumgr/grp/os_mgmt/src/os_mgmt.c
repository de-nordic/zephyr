...
#elif defined(MCUBOOT_BOOTLOADER_MODE_SWAP_WITHOUT_SCRATCH)
#define BOOTLOADER_MODE OS_MGMT_BOOTLOADER_INFO_MCUBOOT_MODE_SWAP_WITHOUT_SCRATCH
#elif defined(MCUBOOT_BOOTLOADER_MODE_SWAP_SCRATCH)
#define BOOTLOADER_MODE OS_MGMT_BOOTLOADER_INFO_MCUBOOT_MODE_SWAP_SCRATCH
#elif defined(MCUBOOT_BOOTLOADER_MODE_DIRECT_XIP)
...
os_mgmt_bootloader_info(struct smp_streamer *ctxt)
{
	...
	zcbor_state_t *zsd = ctxt->reader->zs;
	size_t decoded;
	int bootloader_mode = OS_MGMT_BOOTLOADER_INFO_MCUBOOT_MODE_UNKNOWN;
	bool ok;

	struct zcbor_map_decode_key_val bootloader_info[] = {
		ZCBOR_MAP_DECODE_KEY_DECODER("mode", zcbor_int32_decode, &bootloader_mode),
	};
