 ...
 * work more effectivley, or if device has a support for internal fill
 ...
 *  program-erase devices; in case when code relies on erasing
 *  device, i.e. setting it to erase-value, prior to some operations,
 *  but should work with program-erase and RAM non-volatile devices,
 ...
 *
 *  If device is program-erase type device or device driver provides erase
 *  callback, the callback of the device is called, in which it behaves
 *  the same way as flash_erase.
 *  If a device is non-volatile RAM, so not a program-erase type,
 ...
 *  device.
 *
 *  Erase page offset and size are constrains of program-erase devices,
 *  but can be relaxed for RAM non-volatile devices, which means that
 ...
 *  program-erase type device, but may succeed on RAM non-volatile
 *  device.
