 ...
 */
#define FPF_NON_UNIFORM_LAYOUT	0x01	/**
					 * Pages across device have various sizes.
...
*
* Returns total number of pages available on device; in case of virtual devices that are incorrectly
* configured to split pages at the beginning or the end of range, this will return number of
* complete pages.
*
* @param[in] dev	flash device.
*
...
*/
typedef size_t (*flash_api_get_page_count)(const struct device *dev);
 ...
 */
typedef size_t (*flash_api_get_size)(const struct device *dev);
#endif /* CONFIG_FLASH_PAGE_LAYOUT */
...
static int z_impl_flash_get_page_info(const struct device *dev, off_t off, struct flash_page_info *fpi)
{
...
static size_t z_impl_flash_get_page_count(const struct device *dev)
{
	const struct flash_driver_api *api = (const struct flash_driver_api *)dev->api;
...
static size_t z_impl_flash_get_size(const struct device *dev)
{
	const struct flash_driver_api *api = (const struct flash_driver_api *)dev->api;
