...
int z_impl_flash_flatten(const struct device *dev, off_t offset, size_t size)
{
	...
	if (IS_ENABLED(CONFIG_FLASH_HAS_ERASE_REQUIREMENT) && api->erase) {
		rc = api->erase(dev, offset, size);
	} else if (IS_ENABLED(CONFIG_FLASH_HAS_NO_ERASE_REQUIREMENT)
		   && api->erase == NULL) {
		const struct flash_parameters *fparams = api->get_parameters(dev);

		printk("Filling\n");
		rc = flash_fill(dev, fparams->erase_value, offset, size);
	}
