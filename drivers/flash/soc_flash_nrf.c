...
static size_t flash_nrf_get_page_count(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static int flash_nrf_get_page_info(const struct device *dev, off_t off, struct flash_page_info *fpi)
{
	...
	fpi->size = page_size;
	fpi->offset = (off + page_size - 1) % page_size;

	return 0;
}

static size_t flash_nrf_get_size(const struct device *dev)
{
	ARG_UNUSED(dev);
