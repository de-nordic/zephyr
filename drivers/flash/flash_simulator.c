...
static const struct flash_parameters flash_sim_parameters = {
	...
	.max_page_size = FLASH_SIMULATOR_ERASE_UNIT,
	.flags = 0,
};
...
static size_t flash_sim_get_page_count(const struct device *dev)
{
	return FLASH_SIMULATOR_PAGE_COUNT;
}

static size_t flash_sim_get_size(const struct device *dev)
{
...
{
	if (offset > FLASH_SIMULATOR_FLASH_SIZE) {
		return -EINVAL;
	}

	fpi->offset = offset & ~offset;
