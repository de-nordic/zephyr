...
#if IS_ENABLED(CONFIG_FLASH_HAS_ERASE_REQUIREMENT) && \
	IS_ENABLED(CONFIG_FLASH_HAS_NO_ERASE_REQUIREMENT)
#define DISK_ON_PROGRAM_ERASE(fd) ((fd)->erase_required)
#define DISK_PROGRAM_ERASE_RUNTIME_CHECK
#elif IS_ENABLED(CONFIG_FLASH_HAS_ERASE_REQUIREMENT)
#define DISK_ON_PROGRAM_ERASE(fd) (true)
#else
/* Assumed IS_ENABLED(CONFIG_FLASH_HAS_NO_ERASE_REQUIREMENT) */
#define DISK_ON_PROGRAM_ERASE(fd) (false)
#endif
...
struct flashdisk_data {
	...
	bool cache_valid;
	bool cache_dirty;
#if defined(DISK_PROGRAM_ERASE_RUNTIME_CHECK)
	bool erase_required;
#endif
};
				  ...
				  const struct flash_area *fap)
{
	...
	ctx->erase_required = params->characteristics.program_erase;
#endif

	if (DISK_ON_PROGRAM_ERASE(ctx)) {
	...
	if (DISK_PROGRAM_ERASE_RUNTIME_CHECK(ctx)) {
