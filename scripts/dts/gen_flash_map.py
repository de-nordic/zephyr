...
def get_partition_size(node):
    ...
    if 'reg' not in node.props:
        # Log a warning but don't fail - the DT macros will handle this
        return 0
 ...
 * 
 * This file replaces the macro-based generation in flash_map_default.c.
 * It generates:
 * - default_flash_map[] array with all flash partitions and subpartitions
 * - Individual partition objects (global_fixed_partition_ORD_<N>)
 * - Individual subpartition objects (global_fixed_subpartition_ORD_<N>)
 * 
 * The FIXED_PARTITION_* macros in flash_map.h continue to work as before,
 * referencing the partition objects generated here.
 */
